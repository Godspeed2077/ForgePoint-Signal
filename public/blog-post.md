...
- **Ingest** — A GitHub Actions cron runs daily at 13:15 UTC. It pulls the last 90 days of estate-tax and gift-tax documents from the Federal Register API and the latest IRS Internal Revenue Bulletin issues (parsing Rev. Rul. / Rev. Proc. / Notice / Ann. citations from each weekly issue page), filters for estate/gift/trust/inheritance/Form 706/Form 709/generation-skipping keywords, and dedupes by source URL.
...
- More data sources beyond Federal Register and IRS Internal Revenue Bulletin: state-level revenue departments, Tax Court opinions. The parsing pipeline is source-agnostic — adding a source is a ~50-line diff.
